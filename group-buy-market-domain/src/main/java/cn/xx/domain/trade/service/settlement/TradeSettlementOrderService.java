...
public class TradeSettlementOrderService implements ITradeSettlementOrderService {
    ...
    public Map<String, Integer> execSettlementNotifyJob(String teamId) throws Exception {
        log.info(
                "拼团交易-执行结算通知回调，指定 teamId:{}",
                teamId
        );

        List<NotifyTaskEntity> notifyTaskEntityList =
                repository.queryUnExecutedNotifyTaskList(teamId);

        return execSettlementNotifyJob(notifyTaskEntityList);
    }

    private Map<String, Integer> execSettlementNotifyJob(
            List<NotifyTaskEntity> notifyTaskEntityList)
            throws Exception {
        ...
        for (NotifyTaskEntity notifyTask : notifyTaskEntityList) {
            ...
            } else if (NotifyTaskHTTPEnumVO.ERROR
                    .getCode()
                    .equals(response)) {
